<?php
// news_create.php - Create new news article
session_start();
...
$status = 'draft'; // Default status for initial page load
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $sql = "INSERT INTO news (title, excerpt, content, category, status, image_url, author_id, published_at, created_at) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())";
            $stmt = dbPrepare($sql);
            
            // SIMPLIFIED: Always set status to published if publish button clicked
            // Use published_at date for filtering instead of scheduled status
            if ($status === 'published') {
                if (!empty($publish_date)) {
                    // Use the specified publish date
                    $published_at = date('Y-m-d H:i:s', strtotime($publish_date));
                } else {
                    // Use current date/time if no specific date provided
                    $published_at = date('Y-m-d H:i:s');
                }
            } else {
                // For drafts, set to null
                $published_at = null;
            }
            
            $stmt->bind_param('ssssssis', $title, $excerpt, $content, $category, $status, $image_url, $user['id'], $published_at);
            $stmt->execute();
            ...
            $errors[] = 'Error creating news article: ' . $e->getMessage();
        }
    }
}
?>
            ...
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h2 class="h4 mb-0">Create News Article</h2>
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="form-text">Optional: Upload news image (max 5MB)</div>
                            </div>
                            ...
                            <div class="d-flex justify-content-end gap-2">
                                ...
                                <button type="submit" name="publish" value="1" class="btn btn-primary">Publish</button>
                            </div>
                        </form>
                    </div>
                </div>
