<?php
// event_edit.php - Edit existing event
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            $sql = "UPDATE events SET title = ?, description = ?, content = ?, event_date = ?, 
                    event_time = ?, location = ?, event_type = ?, status = ?, image_url = ? 
                    WHERE id = ?";
            $stmt = dbPrepare($sql);
            $stmt->bind_param('sssssssssi', $title, $description, $content, $event_date, 
                             $event_time, $location, $event_type, $status, $image_path, $event_id);
            $stmt->execute();
